...
class Application:
    ...
    def __init__(self):
        ...
        self.T2.config(text = "Word :", font = ("Times New Roman", 30, "bold"))
        ...
        self.T4.config(text = "Suggestions :", fg = "black", font = ("Times New Roman", 30, "bold"))
    ...
    def video_loop(self):
        ...
        if ok:
            ...
            self.panel5.config(text = self.str,font = ("Times New Roman", 30))

            predicts = self.hs.suggest(self.word)
            
            if(len(predicts) > 1):

                self.bt1.config(text = predicts[0], font = ("Times New Roman", 20))
            ...
            if(len(predicts) > 2):
                ...
                self.bt2.config(text = predicts[1], font = ("Times New Roman", 20))
            ...
            if(len(predicts) > 3):
                ...
                self.bt3.config(text = predicts[2], font = ("Times New Roman", 20))
    ...
    def predict(self, test_image):
        ...
        test_image = cv2.resize(test_image, (85, 85))

        result = self.loaded_model.predict(test_image.reshape(1, 85, 85, 1))

        result_dru = self.loaded_model_dru.predict(test_image.reshape(1 , 85 , 85 , 1))

        result_tkdi = self.loaded_model_tkdi.predict(test_image.reshape(1 , 85, 85 , 1))

        result_smn = self.loaded_model_smn.predict(test_image.reshape(1 , 85, 85 , 1))
